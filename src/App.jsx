  ...
  try {
    ...
    const files = parsed
      ...
      .map((file) => ({
        ...
        name: file.name.replace(/\.ASM$/i, "") || "UNTITLED",
...
function sanitizeFileName(value) {
  return value.toUpperCase().replace(/\.ASM$/i, "").replace(/[^A-Z0-9_-]/g, "_").trim();
}
...
export default function App() {
  ...
  const handleOpenFile = (fileId) => {
    ...
    if (file) {
      ...
      setToast({ kind: "success", text: `Opened ${file.name}.asm` });
    }
  };
  ...
  const handleCreateFile = () => {
    ...
    if (files.some((file) => file.name === name)) {
      setToast({ kind: "error", text: `File ${name}.asm already exists.` });
      return;
    }
    ...
    setToast({ kind: "success", text: `Created ${name}.asm` });
  };
  ...
  const handleRenameFile = (fileId) => {
    ...
    if (files.some((file) => file.id !== fileId && file.name === nextName)) {
      setToast({ kind: "error", text: `File ${nextName}.asm already exists.` });
      return;
    }
    setFiles((prev) => prev.map((file) => (file.id === fileId ? { ...file, name: nextName } : file)));
    setToast({ kind: "success", text: `Renamed file to ${nextName}.asm` });
  };
  ...
  const handleDeleteFile = (fileId) => {
    ...
    if (files.length === 1) {
      setToast({ kind: "error", text: "At least one .asm file must remain in the workspace." });
      return;
    }
    const confirmed = window.confirm(`Delete ${target.name}.asm?`);
    if (!confirmed) {
      return;
    }
    ...
    setToast({ kind: "success", text: `Deleted ${target.name}.asm` });
  };
  ...
  const handleExportFile = useCallback(() => {
    ...
    saveTextFile(`${activeFile.name}.asm`, activeFile.content);
    setDirtyFileIds((prev) => prev.filter((id) => id !== activeFile.id));
    setToast({ kind: "success", text: `Exported ${activeFile.name}.asm` });
  }, [activeFile]);
  ...
  const handleImportFile = async (event) => {
    ...
    try {
      ...
      setDirtyFileIds((prev) => prev.filter((id) => id !== newFile.id));
      setBottomTab("listing");
      setToast({ kind: "success", text: `Imported ${newFile.name}.asm` });
    } catch {
    ...
    <main className={`app-shell ${isRunning ? "app-running" : ""} ${isAppEntering ? "app-entering" : ""}`}>
      ...
      <header className="hero compact-hero">
        ...
        <div className="header-tools">
          <input ref={importInputRef} type="file" accept=".asm,.txt,text/plain" className="hidden-file-input" onChange={handleImportFile} />
      ...
      <section className="workspace-shell">
        <FileSidebar
          ...
          onImportFile={handleOpenImportPicker}
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      ...
                      role="button"
                      tabIndex={0}
                      aria-label={`Close ${file.name}.asm`}
                      onClick={(event) => {
            ...
            <div className="left-column">
              <section className="panel tall-panel">
                <div className="section-title-row">
                  ...
                  <span className="chip">{activeFile ? `${activeFile.name}.ASM` : "NO OPEN TAB"}</span>
                </div>
                {activeFile ? (
                  <>
                  ...
                  <div className="editor-empty-state">
                    <div className="editor-empty-copy">
                      ...
                      <p>Create a new assembly file, import an existing `.asm` file, or load a simple sample program to start exploring Codembly.</p>
                    </div>
              ...
              <section className="panel inspector-shell">
                <section className="inspector-pane">
                  <div className="inspector-toolbar">
                    <div className="inspector-subtab-strip inspector-subtab-strip-quad">
                      <button type="button" className={`inspector-subtab ${inspectorView === "steps" ? "is-active" : ""}`} onClick={() => setInspectorView("steps")}>
                        Steps
                      </button>
